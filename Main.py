...
def process_page_batch(file_path, page_range, output_dir):
    """Process a batch of pages in the given range."""
    with pdfplumber.open(file_path) as pdf:
        for page_num in page_range:
            if page_num >= len(pdf.pages):
                break  # Skip if range exceeds total pages
            ...
            # Save image
            page_image = page.to_image()  # Create a PageImage object
            image = page_image.original # Render the image as a Pillow object
...
def process_pdf_in_batches(file_path, output_dir, batch_size=30):
    """Process the PDF file in parallel by batches of pages."""
    with pdfplumber.open(file_path) as pdf:
        num_pages = len(pdf.pages)

    # Create page ranges for batching
...
def handle_file_processing(file_path, output_dir):
    ...
    process_pdf_in_batches(file_path, output_dir, batch_size=20)  # Adjust batch_size as needed
...
def upload_file():
    ...
    # Save each file and start background processing
    for file in uploaded_files:
        # Save the uploaded file directly to the output directory
